...
export class AuthService {
   ...
   *
   * 1) registerWithEmail
   *  - email, nickname, password를 받아서 새로운 사용자를 생성한다.
   *  - 생성이 완료되면, accessToken, refreshToken을 발급 후 반환한다.
   *
   * 2) loginWithEmail
   *  - email, password를 입력하면 사용자 검증을 진행한다.
   *  - 검증이 완료되면, accessToken, refreshToken을 발급 후 반환한다.
   *
   * 3) loginUser
  ...
  signToken(user: Pick<UsersModel, 'email' | 'id'>, isRefreshToken: boolean) {
    ...
    return this.jwtService.sign(payload, {
      secret: this.configService.get<string>(ENV_JWT_SECRET_KEY),
      expiresIn: isRefreshToken ? 31557600000 : 31557600000
    });
  }
  ...
  async authenticateWithEmailAndPassword(
     ...
     *
     * 1) 입력된 비밀번호
     * 2) 기존 해시(hash) -> 사용자 정보에 저장되어 있는 해시
     */
